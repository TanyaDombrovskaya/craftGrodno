...
function openEditModal(productId, productName, productAbout, productPrice, productCount) {
    ...
    if (productCard) {
        ...
        if (productImage && productImage.src && !productImage.src.includes('data:,')) {
            preview.innerHTML = `<img src="${productImage.src}" alt="Текущее изображение" style="max-width: 100%; max-height: 150px; border-radius: 6px;">`;
        } else {
        ...
        imageInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    preview.innerHTML = `<img src="${e.target.result}" alt="Новое изображение" style="max-width: 100%; max-height: 150px; border-radius: 6px;">`;
                }
...
function handleEditClick(e) {
    ...
    const productAbout = btn.dataset.productAbout;
    const productPrice = btn.dataset.productPrice;
    const productCount = btn.dataset.productCount;
...
function handleDeleteClick(e) {
    const btn = e.target;
    // Эта функция будет обрабатываться deleteProduct.js
}
